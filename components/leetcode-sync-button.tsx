  ...
  if (variant === "leetcode") {
    return (
      <button
        ...
        onClick={handleClick}
        disabled={isPending}
        aria-label={solved ? "Today's LeetCode POTD solved" : "Check today's LeetCode submission"}
        className={cn(
          "shrink-0 rounded-full px-5 py-2 text-xs font-bold uppercase tracking-wide transition-all duration-200",
          "text-[#1A1A1A] hover:-translate-y-0.5 hover:brightness-110",
          "active:translate-y-0 active:brightness-95 disabled:opacity-60 disabled:hover:translate-y-0",
          solved
            ? "bg-[#2CBB5D] shadow-[0_2px_10px_rgba(44,187,93,0.35)] hover:shadow-[0_6px_18px_rgba(44,187,93,0.5)]"
            : "bg-[#FFA116] shadow-[0_2px_10px_rgba(255,161,22,0.35)] hover:shadow-[0_6px_18px_rgba(255,161,22,0.5)]"
        )}
      >
